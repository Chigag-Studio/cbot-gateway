...
DOCKER_IMAGE="daldiv/cbot-gateway"
...
read -p "   Enter the location where your Cbot files are located (example: /Users/cbot/cbot_files) >>> " FOLDER
if [ "$FOLDER" == "" ]
then
  prompt_hummingbot_data_path
else
...
prompt_terra_network () {
  ...
  then
    ...
    TERRA_CHAIN="columbus-5"
  elif [ "$TERRA" == "testnet" ]
  then
    TERRA_LCD_URL="https://bombay-lcd.terra.dev"
    TERRA_CHAIN="bombay-12"
  fi
}
...
echo
printf "%30s %5s\n" "Cbot Instance ID:" "$HUMMINGBOT_INSTANCE_ID"
