...
list_instances () {
 ...
 echo
 docker ps -a --filter ancestor=$DOCKER_IMAGE:$TAG
 echo
}
...
execute_docker () {
 ...
 # 2) Delete old image
 echo
 read -p "   Delete current docker image $DOCKER_IMAGE:$TAG? [Y/N] >>> " DELETE_IMAGE
 if [[ "$DELETE_IMAGE" == "Y" || "$DELETE_IMAGE" == "y" ]]
 then
  echo
  echo "Deleting old image: $DOCKER_IMAGE:$TAG"
  docker image rm $DOCKER_IMAGE:$TAG
  echo
 fi
 ...
 #3 ) Pull docker image
 echo
 read -p "   Pulling docker image $DOCKER_IMAGE:$TAG [Y/N] >>> " PULL_IMAGE
